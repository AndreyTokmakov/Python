...
# 3. As a result, there are two dictionaries 'teethMap'
#    and 'teethMapOriented' in format of Dict[ToothID, vtkPolyData]
# 4 teethMapOriented --> same as but rotated around X and Y axes
class ToothOBJFileReader(object):

    def __init__(self):
        self.teethMap: Dict[int, vtkPolyData] = {}
        self.teethMapOriented: Dict[int, vtkPolyData] = {}
    ...
    def parse_crowns_data_lines(self, lines: List[str]) -> None:
        lines = tuple(filter(ToothOBJFileReader.is_not_empty, lines))
        if lines is None:
            raise ValueError(f'Empty file')
        obj_split = ToothOBJFileReader.__split_lines(lines)

        self.teethMap, self.teethMapOriented = {}, {}
        ...
        while i < len(lines) and split_res[0] == 'g':
            ...
            while i < len(lines) and split_res[0] == 'o':
                ...
                while i < len(lines) and (split_res[0] in {'v', 'f', 'vn'}):
                    vec = ToothOBJFileReader._vector_from_obj_str_split(split_res, len(geometry['v']))
                ...
                self.teethMapOriented[toothId] = Utilities.rotatePolyData(res, 90, 180)
    ...
    def decimate_teeth_obj(self,
                           outPath: str):
        teeth: Dict[int, vtkPolyData] = {k: ToothOBJFileReader.decimate_poly_data(v) for k, v in self.teethMap.items()}
        ToothOBJFileReader.write_obj_teeth(teeth, outPath)